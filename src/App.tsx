...
function App() {
  ...
  const NAV_ITEMS: { id: Vista; label: string; icon: string; subtitulo: string }[] = [
    { id: "facturas", label: "Facturas", icon: "📄", subtitulo: "Listado y seguimiento de DTEs cargados" },
    { id: "dashboard", label: "Dashboard", icon: "📊", subtitulo: "Resumen financiero y alertas" },
    { id: "clientes", label: "Clientes", icon: "👥", subtitulo: "Cartera de clientes y contactos" },
    { id: "calendario", label: "Calendario", icon: "🗓️", subtitulo: "Eventos de facturación, vencimiento y pago" },
    { id: "configuracion", label: "Configuración", icon: "⚙️", subtitulo: "Alertas, plantillas y sincronización SII" },
  ];
  const vistaActual = NAV_ITEMS.find((n) => n.id === vista)!;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-100 text-[13px]">
      <aside className="flex w-52 flex-col border-r border-slate-300 bg-slate-900">
        <div className="flex items-center gap-2 border-b border-slate-700 px-4 py-4">
          ...
          <div>
            <p className="text-[13px] font-bold leading-tight text-white">Facturas SII</p>
            <p className="text-[11px] text-slate-400">Gestión de cobranza</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-0.5 px-2 py-2">
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className={`flex w-full items-center gap-2 rounded px-2.5 py-2 text-[13px] font-medium transition ${
                vista === item.id ? "bg-blue-600 text-white" : "text-slate-300 hover:bg-slate-800 hover:text-white"
              }`}
            >
        ...
        <div className="border-t border-slate-700 p-2">
          {facturas.length > 0 && permisos.limpiarFacturas && (
            <button
              onClick={handleClear}
              className="mb-1 w-full rounded px-2.5 py-1.5 text-left text-[13px] text-slate-300 hover:bg-red-950 hover:text-red-300"
            >
              Limpiar todo
            </button>
          )}
          <div className="flex items-center gap-2 rounded px-2 py-1.5">
            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-700 text-[11px] font-semibold text-slate-200">
              {iniciales}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[12px] font-medium text-white">{usuario.nombre}</p>
              <p className="text-[11px] text-slate-400">{ROLE_LABEL[usuario.role]}</p>
            </div>
            <button onClick={handleLogout} title="Salir" className="text-slate-400 hover:text-white">
              ⏻
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-300 bg-white px-5 py-3">
          <div>
            <h1 className="text-base font-bold leading-tight text-slate-900">{vistaActual.label}</h1>
            <p className="text-[12px] text-slate-500">{vistaActual.subtitulo}</p>
          </div>
        </div>
