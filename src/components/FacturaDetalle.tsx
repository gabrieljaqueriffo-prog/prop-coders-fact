      ...
      >
        ...
        <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded border border-slate-200 p-3">
            ...
            <p className="text-sm text-slate-600">{formatRUT(factura.emisor.rut)}</p>
            <p className="text-sm text-slate-600">{factura.emisor.giro}</p>
            <p className="text-sm text-slate-600">{factura.emisor.email}</p>
            <p className="text-sm text-slate-600">{factura.emisor.direccion}</p>
          </div>
          <div className="rounded border border-slate-200 p-3">
            <h3 className="mb-1 text-xs font-semibold uppercase text-slate-500">Receptor</h3>
            <p className="font-medium">{factura.receptor.nombre}</p>
            <p className="text-sm text-slate-600">{formatRUT(factura.receptor.rut)}</p>
            <p className="text-sm text-slate-600">{factura.receptor.giro}</p>
            <p className="text-sm text-slate-600">{factura.receptor.direccion}</p>
          </div>
        </div>

        <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Detalle</h3>
        <div className="mb-4 overflow-x-auto rounded border border-slate-200">
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Ítem</th>
                <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Descripción</th>
                <th className="px-3 py-2 text-right text-xs font-semibold uppercase text-slate-500">Cantidad</th>
                <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Unidad</th>
                <th className="px-3 py-2 text-right text-xs font-semibold uppercase text-slate-500">Precio Unit.</th>
        ...
        <div className="mb-4 ml-auto w-full max-w-xs space-y-1 rounded border border-slate-200 p-3 text-sm sm:ml-auto">
          <div className="flex justify-between">
            <span className="text-slate-500">Neto</span>
            <span>{formatCLP(factura.totales.neto)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">IVA ({factura.totales.tasa}%)</span>
        ...
        {factura.referencias.length > 0 && (
          <div className="mb-4">
            <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Referencias</h3>
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                ...
                <div>
                  <label className="text-xs text-slate-400">Plazo (días)</label>
                  <input
                    type="number"
                    min={0}
                    value={plazoInput}
                  ...
                  />
                  {esCesionAutoritativa && <p className="text-xs text-slate-400">Definido por cesión</p>}
                </div>
                <div>
                  <p className="text-xs text-slate-400">Vencimiento {esCesionAutoritativa ? "(cesión)" : "estimado"}</p>
                ...
                <div className="col-span-2 sm:col-span-4">
                  {gestion?.fechaPago ? (
                    ...
                    <p className={`text-sm ${diasHastaVencimiento < 0 ? "text-red-600" : "text-slate-600"}`}>
                      {diasHastaVencimiento < 0
        ...
        <div className="mb-4 rounded border border-slate-200 p-3">
          <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Gestión de cobro</h3>
          ...
          <select
            ...
            disabled={readOnly}
            className="mb-3 w-full rounded border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-100"
          >
          ...
          <label className="mb-1 block text-xs font-semibold uppercase text-slate-500">Notas internas</label>
          <textarea
            value={notas}
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {templates.map((t) => (
              <button
                ...
                disabled={readOnly}
                className="rounded border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
          ...
          {logs.length > 0 && (
            <div>
              <h4 className="mb-1 text-xs font-semibold uppercase text-slate-500">Historial</h4>
