    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-start justify-between border-b border-slate-200 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {!esNuevo && <p className="text-sm text-slate-500">{formatRUT(form.rut)}</p>}
              {!readOnly && (
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-slate-500">WhatsApp</label>
            <input
              value={form.whatsapp}
          ...
          <>
            <div className="mb-4 rounded border border-slate-200 p-3">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-xs font-semibold uppercase text-slate-500">Análisis de riesgo de pago</h3>
                <span className={`rounded-full px-2 py-1 text-xs font-medium ${RIESGO_BADGE[riesgo.nivel]}`}>
                  {RIESGO_LABEL[riesgo.nivel]}
                </span>
              </div>
              <p className="mb-2 text-sm text-slate-600">{riesgo.mensaje}</p>
              <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                ...
                <div>
                  <p className="text-xs text-slate-400">Pagadas</p>
                  <p className="font-medium">{riesgo.pagadas}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Vencidas / en mora</p>
                ...
                <div>
                  <p className="text-xs text-slate-400">Pagos atrasados</p>
                  <p className="font-medium">{riesgo.pagosAtrasados}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Atraso promedio</p>
                  <p className="font-medium">
            ...
            <div className="mb-4">
              <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Historial de facturación</h3>
              <div className="overflow-x-auto rounded border border-slate-200">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Folio</th>
                      ...
                      <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Gestión</th>
                      <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-500">Fecha pago</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {facturasCliente.map((f) => {
                      ...
                      return (
                        <tr key={facturaId(f)}>
                          ...
                          <td className="px-3 py-2 text-slate-600">{gestion?.fechaPago ?? "—"}</td>
                          {onVerFactura && (
                    ...
                    {facturasCliente.length === 0 && (
                      <tr>
                        <td colSpan={onVerFactura ? 7 : 6} className="px-3 py-6 text-center text-slate-400">
